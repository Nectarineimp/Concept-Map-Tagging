#####
# TaggerDesktop.py: For exploration of text files with tagging
# Run with Python 2.6.6, nltk 2, numpy 1.6, PyYAML 3.
# ex.: c:\python26\python c:\bin\TaggerDesktop.py c:\textfiles
# ARGUMENTS: (1) directory of text files (may contain subdirectories of text files too.)
...
def ConceptMap(sentences, filelocation):
...
def StoreConceptMap(ConceptMap):
    ...
    if RSD >= PASSFAIL:        
        ...
        for gram, freq in NGRAM[0:NGC]:
            if freq > 1:
                print '{0}\t{1}\t{2}\t{3}\t{4}'.format(recordnum, gram, 'C', filelocation, freq)
                recordnum += 1
        for gram, freq in BIGRAM[0:BGC]:
            if freq > 1:
                print '{0}\t{1}\t{2}\t{3}\t{4}'.format(recordnum, gram, 'B', filelocation, freq)
                recordnum += 1
        for gram, freq in UNIGRAM[0:UGC]:
            if freq > 1:
                print '{0}\t{1}\t{2}\t{3}\t{4}'.format(recordnum, gram, 'U', filelocation, freq)
                recordnum += 1


def loadSignalPattern():
    #finds the files, filter noise, tokenize, parse, detect POS
    corpuslocation = sys.argv[1]
    if not os.path.exists(corpuslocation):
        print "Corpus location", corpuslocation, "does not appear to exist."
        quit(-1)
    else:
        for (path, dirs, files) in os.walk(corpuslocation):
            for file in files:
                fh = open(path+'\\'+file,'r')
                plaintext = fh.read()
                fh.close()
                ...
                StoreConceptMap(ConceptMap(sentences, path+'\\'+file))
    return(1)
